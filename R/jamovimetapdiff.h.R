...
jamovimetapdiff <- function(
    ...
    if ( ! missing(reference_cases)) reference_cases <- jmvcore::resolveQuo(jmvcore::enquo(reference_cases))
    if ( ! missing(reference_ns)) reference_ns <- jmvcore::resolveQuo(jmvcore::enquo(reference_ns))
    if ( ! missing(comparison_cases)) comparison_cases <- jmvcore::resolveQuo(jmvcore::enquo(comparison_cases))
    ...
    if (missing(data))
        data <- jmvcore::marshalData(
            parent.frame(),
            `if`( ! missing(reference_cases), reference_cases, NULL),
            `if`( ! missing(reference_ns), reference_ns, NULL),
            `if`( ! missing(comparison_cases), comparison_cases, NULL),
